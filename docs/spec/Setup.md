...
- Hugging Face repo、immutable revision、锁定成品资产路径、字节数与 SHA-256；
- 成品资产导入器 commit、导入配置及其 SHA-256；
- 坐标系、单位、量化和区间转换规则；
...
成品资产下载、校验与确定性导入不属于算法 measured run，不使用 900-second benchmark timeout；其失败使对应数据集状态记为 `DATASET-CONSTRUCTION-FAILED`，并跳过依赖该 workload 的实验。脚本至少生成：
...
本实验明确把下表锁定 revision 的三份 Hugging Face 成品数据集作为唯一真实数据输入。raw-to-final 重建属于上游数据集发布流程，不属于本实验仓库、`prepare_real_data.sh` 或 `run_all_lite.sh` 的职责；本仓库只执行可复现的成品资产下载、完整性验证、实验子集导入和规范二进制 workload 写出。
...
4. 只对成品表执行实验子集导入：
   - CMAB 以发布字段 `(source_file, source_fid)` 作为跨四个广东 level 的唯一源记录键，直接读取已发布的 base/expanded AABB 与 function code，再作固定的 function-class/10 km tile 分层哈希划分；`building_uid` 仅作为带碰撞诊断的发布字段，不作为对象键或去重依据。
...
| Dataset | 冻结端点 | 半开语义 |
|---|---|---|
| CMAB | 固定投影平面中的 binary64 AABB | 成品表中已发布的连续 AABB 直接解释为 $[L,U)$；边界相接不计相交。 |
...
仓库直接读取成品数据集中已经投影和构造好的四个广东省 level，不读取原始建筑几何，也不在本仓库执行 CRS 转换。发布坐标是以 meter 为单位的 Albers Equal Area 平面 binary64；仓库将其冻结标识为 `CMAB-HF-Albers-Equal-Area-m`。四个 level 必须各有 943285 行，并按唯一复合键 `(source_file, source_fid)` 一一对齐；base AABB、function code 与建筑身份必须跨 level 一致。
...
成品表为每个建筑物提供该投影平面中的 binary64 base AABB：
...
H_{\mathrm{stratum}}(G)
=
\operatorname{StableHash}(\texttt{cmab-hf-stratum-v1},G)
$$
...
=
\operatorname{StableHash}(\texttt{cmab-hf-building-split-v1},\mathrm{canonical\_id}(b)).
$$

冻结的 Hugging Face 广东分区中 `building_uid` 存在 4898 组二重碰撞，不能直接作为对象键，也不能据此去重。导入器以发布字段 `(source_file, source_fid)` 为规范源记录键：先验证该复合键在 943285 行和四个 level 上唯一且完全一致，再按复合键字典序排序，并把排序位置 `0..943284` 定义为 `canonical_id`。每个 stratum 内按 $H_{\mathrm{building}}(b)$ 升序排序，前 $q_G$ 个对象进入 $B_R$，其余对象进入 $B_S$；manifest 保存源键序列 SHA-256、938387 个不同发布 UID、4898 个碰撞组和最大重数 2。
...
| `object_identity` | `(source_file, source_fid)`、`uint64_lexicographic_rank_v1` 与源键序列 SHA-256。 |
| `published_building_uid_diagnostics` | 不同 UID 数、碰撞组数、超额行数与最大重数。 |
...
$$
H_{\mathrm{user}}(u)=
\operatorname{StableHash}(\texttt{geolife-hf-user-split-v1},u).
$$
...
=(\operatorname{traj\_id},\operatorname{point\_idx}).
$$

在 $U_R$ 和 $U_S$ 两侧分别执行 temporal-spatial stratified deterministic selection。`month(p)` 使用发布的 UTC epoch milliseconds 对应的 UTC 日历月；1 km 空间 tile 由 centimeter 坐标定义：
...
$$
H_{\mathrm{stratum}}(G)=
\operatorname{StableHash}(\texttt{geolife-hf-stratum-quota-v1},C,G)
$$
...
$$
P_{\mathrm{point}}(p)=
\operatorname{KeyedSplitMix64}(\texttt{geolife-hf-point-priority-v1},\operatorname{point\_key}(p))
$$

按 $(P_{\mathrm{point}}(p),\operatorname{traj\_id},\operatorname{point\_idx})$ 升序选择前 $q_G$ 个 points。最终得到共同点集：
